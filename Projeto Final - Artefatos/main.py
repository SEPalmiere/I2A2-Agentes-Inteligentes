#!/usr/bin/env python3
# =============================================================================
# SCRIPT FINAL - EXTRAÇÃO CONSOLIDADA COM CAMPOS OBRIGATÓRIOS
...
# =============================================================================
# PARTE 2: EXTRACTOR PRINCIPAL
# =============================================================================

class NFEExtractorCompleto:
    """Extrator completo com garantia de campos obrigatórios"""
    
    # ✅ Campos obrigatórios que DEVEM estar no resultado final
    CAMPOS_OBRIGATORIOS = [
        'DATA EMISSÃO',
        'CPF/CNPJ EMITENTE',
    ...
    def setup_directories(self):
        ...
        for dir_path in ['inputs', 'outputs/csv', 'outputs/logs']:
    ...
    def extract_from_csv(self, file_path):
        ...
        try:
            ...
            return self._standardize_list(records)
        except Exception as e:
            print(f"❌ Erro ao extrair CSV {file_path}: {e}")
            return []
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            if data:
                ...
                print(f"✅ {Path(file_path).name}: {len(data)} registros")
    ...
    def get_dataframe_final(self):
        """✅ RETORNA DATAFRAME FINAL COM CAMPOS OBRIGATÓRIOS GARANTIDOS"""
    ...
    def get_statistics(self):
        ...
        return {
            ...
            'total_campos': len(self.STANDARD_FIELDS)
        }
...
def main():
    ...
    print("="*80)
    print("🔍 EXTRATOR DE NOTAS FISCAIS - VERSÃO FINAL CORRIGIDA")
    print("="*80)
    print()
    ...
    # Criar extractor
    extractor = NFEExtractorCompleto()
    
    # Arquivos de entrada
    arquivos_entrada = [
        '/mnt/project/202401_NFs_Cabecalho.csv',
        '/mnt/project/202401_NFs_Itens.csv',
        '/mnt/project/202505_NFe_NotaFiscal.csv'
    ]
    ...
    for arquivo in arquivos_entrada:
        if Path(arquivo).exists():
            n_records = extractor.process_file(arquivo)
            total_processado += n_records
        else:
    ...
    print(f"Arquivos com sucesso: {stats['successful_files']}")
    print(f"Arquivos com erro: {stats['failed_files']}")
    print(f"Total de registros extraídos: {stats['total_records']}")
    print(f"Notas fiscais únicas: {stats['unique_invoices']}")
    print()
    
    print("✅ CAMPOS OBRIGATÓRIOS NO RESULTADO FINAL:")
    print("-" * 80)
    for campo in stats['campos_obrigatorios']:
        print(f"   ✓ {campo}")
    print()
    
    # ✅ Obter dataframe final
    print("📥 Gerando dataframe final...")
    df_final = extractor.get_dataframe_final()
    ...
    if df_final is not None:
        ...
        output_path, df_exported = extractor.export_csv()
        print()
        ...
        # Campos obrigatórios na amostra
        print("⭐ VERIFICAÇÃO DOS CAMPOS OBRIGATÓRIOS:")
        print("-" * 80)
        for campo in extractor.CAMPOS_OBRIGATORIOS:
            tem_campo = campo in df_final.columns
            tem_dados = df_final[campo].notna().sum() > 0 if campo in df_final.columns else False
            status = "✅" if tem_campo and tem_dados else "⚠️" if tem_campo else "❌"
